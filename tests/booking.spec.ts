...
// Use test.beforeEach to log in before every test automatically
test.beforeEach(async ({ page }) => {
    // Assuming Vite runs on localhost:5173
    await page.goto('http://localhost:5173/login');

    await page.getByPlaceholder('Email').fill('patient@clinic.com'); // Adjust to your test user
    await page.locator('input[type="password"]').fill('Uitest@9009');
    await page.getByRole('button', { name: 'Sign In' }).click();

    // 3. Verify routing logic to the patient dashboard
...
test.describe('Patient Appointment Booking Flows', () => {
    ...
    test('Standard Flow: Should book an appointment directly from the marketplace', async ({ page }) => {
        ...
        // 1. Pick Date (Using '29' based on your screenshot)
        await page.locator('button:not([disabled])').filter({ hasText: /^29$/ }).click();

        // 2. Pick Time (Looking for the exact text '09:00' from your screenshot)
    ...
    test('AI Triage Flow: Should recommend a doctor and show the AI banner', async ({ page }) => {
        ...
        // This catches the exact URL your TriageChat.tsx is calling
        ...
        // 3. Wait for the Summary Card to appear (This will now be instant!)
        ...
        // 6. Pick Date (Using '30' to ensure we don't double-book the 29th)
        await page.locator('button:not([disabled])').filter({ hasText: /^30$/ }).click();

        // 7. Pick Time (Looking for '10:00')
